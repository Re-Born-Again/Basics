        ...
        for (char c : s) { // loop through each character in the string
            if (c == '(' || c == '{' || c == '[') { // if the character is an opening bracket
                ...
                if (st.empty() || // if the stack is empty or 
                    ...
                    (c == '}' && st.top() != '{') ||
                    (c == ']' && st.top() != '[')) {
                    cout<<"false"<<endl;; // the string is not valid, so return false
                }
                st.pop(); // otherwise, pop the opening bracket from the stack
            }
        }
        cout<< st.empty()<<endl; // if the stack is empty, all opening brackets have been matched with their corresponding closing brackets,
